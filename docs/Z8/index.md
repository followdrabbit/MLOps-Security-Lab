...
**Objetivo central:** oferecer **controles transversais** (identity, keys, secrets, policy, data protection, logging) que **fortalecem a borda e o runtime** em todo o pipeline (ingestão→serving), viabilizando **minimização de privilégios, rastreabilidade e conformidade**.
**Resultados-chave:**
...
* **SIEM/UEBA/SOAR** centralizam logs/telemetria (Z9) com **trilhas de auditoria**. (Mantém alinhamento de propósito e consumo como em Z7.) 

---

## 2) Componentes da Z8

> Cada item pode ter um arquivo “**Z8-2.x**” de detalhamento, seguindo o mesmo padrão dos documentos de componentes em Z6/Z7. 
...
* **Auditoria** dos acessos a segredos (→ Z9). (Modelo idêntico ao de segredos descrito em Z6.) 
...
* **Catálogo de políticas** versionado (GitOps), *tests* e **evidence links**. (Padroniza o estilo de políticas exemplares visto em Z6/Z7.) 
...
* **Tokenização/mascaramento:** substitui identificadores; **purpose-based access** (só o necessário). (Mesma filosofia “privacy-by-default” reforçada em Z7 UI.) 
...
* **Resposta:** *playbooks* (SOAR), *tickets* e **bloqueios automatizados** (WAF, IAM, chaves). (Alinha com práticas de observabilidade já descritas em Z6/Z7.) 
...
* **Contratos de leitura** (OpenAPI versionado) para Z7 consumir via APIs (não do bucket). 

### 2.8 Compliance, Evidence & Audit Trails — [Detalhamento](./Z8-2.8.md)

* **Trilhas de auditoria** end-to-end (quem, o quê, quando, por quê).
* **Evidence store** (artefatos de build, SBOM, *attestations*, *approvals*).
* **Access reviews** e reconciliação de propósito (LGPD). (Mesma cadência de *access reviews* de Z7.) 
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------- |
| **Segredo exposto / credenciais hardcoded** | 2.2 Vault Agent, *leases* curtos, rotação, OIDC federation (GH→Vault)                      | NIST IA-5/AC-6; OWASP ASVS Secrets; CSA IAM/AAC |
...
| **Autorização fraca / *tenant-escape***     | 2.1 IAM com RBAC/ABAC; 2.4 OPA/Rego (*deny-by-default*; *claims*-based)                    | OWASP API (API1/5); NIST AC-*/IA-*; CSA IAM     |
...
2. **Rego** decide *allow/deny*; logs → **SIEM**. (Padrão de políticas já utilizado em Z6/Z7.) 
